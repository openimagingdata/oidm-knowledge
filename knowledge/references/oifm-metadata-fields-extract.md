...
sources:
  - id: note
    ...
    title: Finding Model Structured Metadata Fields, notes directory of the next-gen-2026 branch, ACR-RSNA-CDEs
  - id: upstream
    resource: https://gist.github.com/talkasab/2d366555a8c371d8f2e2316f9d39d0e0
    title: The upstream gist the note was copied from, published by the project lead
  - id: edge-metadata
...
Read at commit `1942b06`, the branch code and this document disagree in three places. Which one is authoritative is an open question.

- **Subspecialty codes.** The document lists sixteen, including `AB` for Abdominal Radiology and `VI` for Vascular/Interventional. The code defines seventeen and has neither: it adds `NM` for Nuclear Medicine, `SQ` for Quality Assurance and Quality Improvement, and `VA` for Vascular, and labels the enumeration as RSNA-aligned specialty content codes.[^edge-metadata]
- **Etiology codes.** The document's table lists twenty-five. The code defines twenty-seven, adding a bare `vascular` and a `cardiac` member.[^edge-metadata]
- **The anatomic location code system.** The document says `anatomic_locations` entries use `system="ANATOMICLOCATIONS"`. The conversion method in the `anatomic-locations` package emits `system="anatomic_locations"` in lower case.[^fm-location-model]

The document is otherwise consistent with the code, including all ten body regions, nine modalities, seven entity types, three sex specificity values, nine age stages, six duration values, and seven time course modifiers.[^edge-models]
...
[^note]: Finding Model Structured Metadata Fields, notes directory of the next-gen-2026 branch, ACR-RSNA-CDEs
[^upstream]: The upstream gist the note was copied from, published by the project lead
